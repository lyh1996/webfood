...
public class RankUtil {
	 ...
	 */
	public static void updateScore( Jedis jedis, String key, double score, String user){
		if( key==null){
			key=RANKBOARD;
		}
		if(jedis.zincrby(key, score, user)==0){
			jedis.zadd(key, score,user);
		}
		//return jedis.zadd(key, score, user);
	}
	
	public static void updateScore( Jedis jedis,  double score, String user){
		 updateScore(jedis,null, score, user);
	}
